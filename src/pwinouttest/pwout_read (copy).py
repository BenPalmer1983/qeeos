...
class pwout_read:
  ...
  def read(path):
    ...
    return pwout_read.read_641(d) 
    if(v == "6.4.1"):
      return pwout_read.read_641(d)    
    if(v == "6.6"):
      return pwout_read.read_66(d)
  ...
  def read_641(d):
    ...
    if(pwtype == 'SCF'):
      out = pwout_read.read_scf_641(d, out)
    elif(pwtype == 'RELAX'):
      out = pwout_read.read_relax_641(d, out)
    elif(pwtype == 'VC-RELAX'):
      out = pwout_read.read_vcrelax_641(d, out)
  ...
  def read_66(d):
    ...
    for dn in d:
      if("BFGS Geometry Optimization" in dn):
        a = 1
      if("Final scf calculation at the relaxed structure." in dn):
        b = 1
    if(a == 1 and b == 0):
      pwtype = "RELAX"
    elif(a == 1 and b == 1):
